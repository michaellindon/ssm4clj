...
(defn approx-corr [ls d]
  (let [l (/ (sqrt 5) ls)
        q (/ (* 16 (pow l 5)) 3)]
    (matrix [[0 0 0] [0 0 0] [0 0 (* q d)]])))

(def approx-reverse-corr approx-corr)
...
(defn approx-reverse-regression
  "Returns a regression matrix for length scale ls and delay d"
  [ls d]
  (let [l (/ (sqrt 5) ls)]
    (matrix [[1 (negate d) 0] [ 0 1 (negate d)]
             [(* d l l l -1)
              (negate (* d l l -3))
              (- 1 (* 3 d l))]])))
